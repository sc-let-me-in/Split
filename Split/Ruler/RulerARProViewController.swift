...
class RulerARProViewController: UIViewController {
    ...
    enum MeasurementMode {
        ...
        func toAttrStr() -> NSAttributedString {
            let str = self == .area ? "Area measurement" : "Length measurement"
    ...
    private var lastState: ARCamera.TrackingState = .notAvailable {
        didSet {
            ...
            case .limited(let reason):
                switch reason {
                case .initializing:
                    HUG.show(title: "AR is initializing", message:"Please shake the device to get more feature points", inSource: self, autoDismissDuration: nil)
                case .insufficientFeatures:
                    HUG.show(title: "AR insufficient features", message: "The feature point is not sufficient, please sway the equipment to obtain more feature points", inSource: self, autoDismissDuration: 5)
                case .excessiveMotion:
                    HUG.show(title: "AR excessive motion", message: "Device moves too fast", inSource: self, autoDismissDuration: 5)
                case .relocalizing:
                    HUG.show(title: "AR is initializing", message: "unknown message", inSource: self, autoDismissDuration: 5)
                }
            case .normal:
                HUG.dismiss()
            }
    ...
    private var measureUnit = MeasurementUnit.Unit.foot {
        didSet {
            let v = measureValue
            measureValue = v
        }
...
fileprivate extension RulerARProViewController {
    ...
    func restartSceneView() {
        ...
        measureUnit = MeasurementUnit.Unit.foot
        resultLabel.attributedText = mode.toAttrStr()
        updateFocusSquare()
    }
